...
- Vite v6+ enables `server.preTransformRequests` by default. Currently, the resource paths automatically generated by the plugin are not supported, which may result in errors such as `[vite] Pre-transform error: Failed to load url /src/main.js (resolved id: /src/main.js). Does the file exist?`. However, this does not affect the actual build process. If you are particularly concerned about this error message, you can set `server.preTransformRequests` to false.
